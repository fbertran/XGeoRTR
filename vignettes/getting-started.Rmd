...
workflows. Renderer frontends can consume `xgeo_state` through their own adapter
contracts. Those packages own presentation, interaction, and front-end behavior.

## Downstream figure consumers

`XGeoRTR` stops at backend state and backend tables. The canonical selected
figure assets for the SHAP workflow live downstream in `shapViz3D`, where the
SHAP semantics and figure selection logic are owned.
