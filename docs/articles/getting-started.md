...
summaries for topology-oriented workflows. Renderer frontends such as
`ggWebGL` can consume `xgeo_state` through their own adapter contracts.
Those packages own presentation, interaction, and front-end behavior.
...
The backend-only example below shows the intended downstream-consumption
pattern:
